...
namespace ImageProcessing
{
    ...
    public class BitmapFilter
    {
        ...
        // DFS Function
        public static unsafe void FloodFill(byte* ptr, int stride, int x, int y, bool[,] visited, int width, int height)
        {
            ...
            // Calculate the pixel index for RGB channels
            int pixelIndex = (y * stride) + (x * 3);

            // If the pixel is not part of an edge, return
            byte red = ptr[pixelIndex];
            byte green = ptr[pixelIndex + 1];
            byte blue = ptr[pixelIndex + 2];

            if (red == 0 && green == 0 && blue == 0) // assuming black means no edge
                return;

            // Recursively call FloodFill on the four neighbors (up, down, left, right)
            FloodFill(ptr, stride, x - 1, y, visited, width, height); // Left
            FloodFill(ptr, stride, x + 1, y, visited, width, height); // Right
            FloodFill(ptr, stride, x, y - 1, visited, width, height); // Up
            FloodFill(ptr, stride, x, y + 1, visited, width, height); // Down
        }


        // DFS for FloodFill
        public static unsafe void Dfs(byte* ptr, int x, int y, byte prevClr, byte newClr, int stride, int width, int height)
        {
            // Check if the current pixel is out of bounds or not the target color
            if (x < 0 || y < 0 || x >= width || y >= height)
                return;

            // Calculate the pixel's byte index in memory
            int pixelIndex = (y * stride) + (x * 3);

            // If the pixel is not the previous color, return
            if (ptr[pixelIndex] != prevClr)
                return;

            // Mark the current pixel as the new color
            ptr[pixelIndex] = newClr;

            // Move in the four directions: up, down, left, right
            Dfs(ptr, x - 1, y, prevClr, newClr, stride, width, height);  // Left
            Dfs(ptr, x + 1, y, prevClr, newClr, stride, width, height);  // Right
            Dfs(ptr, x, y - 1, prevClr, newClr, stride, width, height);  // Up
            Dfs(ptr, x, y + 1, prevClr, newClr, stride, width, height);  // Down
        }

        public static int CountBlobs(Bitmap edgeImage)
        {
            int blobCount = 0;
            int width = edgeImage.Width;
            int height = edgeImage.Height;

            Bitmap processedImage = (Bitmap)edgeImage.Clone();

            BitmapData data = processedImage.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
            ...
            unsafe
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    {
                        // Calculate the pixel index for the RGB channels
                        int pixelIndex = (y * stride) + (x * 3);
                        ...
                        // If the pixel is part of an edge (i.e., any channel has a value > 0) and not visited
                        if ((red > 0 || green > 0 || blue > 0) && !visited[y, x])
                        {
                            // Call the FloodFill method to mark the blob and increment the blob count
                            FloodFill(ptr, stride, x, y, visited, width, height);
                            blobCount++;
                        }
                    }
                }
